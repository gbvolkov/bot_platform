    ...
    def run(
        ...
        *,
        ...
        for attempt in range(1, self.config.max_generation_iterations + 1):
            ...
            raw_content = str(generated.get("content", "")).strip()
    ...
    def _review_validation_failure(
        ...
        *,
        ...
        decision = {
            ...
            "non_blocking_issues": [str(item) for item in data.get("non_blocking_issues", [])],
            "overruled_validator_issues": [str(item) for item in data.get("overruled_validator_issues", [])],
            "residual_risks": [str(item) for item in data.get("residual_risks", [])],
            "fix_instructions": [str(item) for item in data.get("fix_instructions", [])],
        }
        self.trace.log(
...
class PackageValidator:
    ...
    def validate(
        ...
        *,
        ...
        package_system_prompt = "You are PackageValidatorAgent. Check the package only. Do not generate or repair materials. Return one JSON object."
        data = self.client.complete_json(
        ...
        result = rule_result.merge(llm_result)
        self.trace.log("package_validation.done", approved=result.approved, issues=result.issues)
        attempt_store.write_package_validation(
            rule_result=rule_result,
            llm_result=llm_result,
            validation=result,
