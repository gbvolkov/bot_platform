...
| Service | What it does | What to pass | What it returns | What it does not do |
|---|---|---|---|---|
| Complete Model Field Profile | Returns every user-facing original BI field available for concrete model records, not a curated "important fields" subset. | Concrete models/modifications/candidate IDs and manufacturer scope such as `manufacturer_brand_lat=GAZ` when known. Ask for all populated non-duplicate user-facing fields. If examples are listed, state they are only a minimum checklist. Ask to exclude only `_nocase` mirror fields, normalized duplicate copies, raw/import/source technical columns, and duplicate aliases. | One profile per concrete model with original user-facing fields, including identity, price, TTX, dimensions, geometry, body/platform, cargo/loading, options/equipment, service/warranty, ownership cost, any other populated user-facing fields, and explicit gaps for user-relevant missing fields. | Does not dump duplicate normalized columns, hide fields outside the current topic, or choose a sales winner. |
| Specific Missing Field Recovery | Re-checks BI when a field needed for the current answer was omitted from a previous BI result. | Active concrete models/modifications/candidate IDs, manufacturer scope when known, missing field names, and likely aliases/schema names such as `front_overhang_mm`, `rear_overhang_mm`, "передний свес", "задний свес". | The requested field values if present, explicit absence if not present, and related original user-facing fields that clarify the result. | Does not replace the complete-profile request for first concrete lookups and does not ask the user to clarify units before checking BI. |
...
| GAZ Catalog Filter | Selects GAZ records by formal parameters. | `manufacturer_brand_lat=GAZ`, body type, mass, payload, passenger capacity, drive, fuel, price, dimensions. | Records matching filters plus missing fields. | Does not invent "best variants" or assign sales roles like "urban", "compromise", "best value". |
...
| Candidate Comparison Table | Produces a factual comparison table for concrete candidates. | List of models/modifications and fields to compare. | Table with model, price, body, mass, payload, engine, options, service data, and gaps. | Does not choose a sales winner unless the winning criterion is formal. |
| Competitor Fact Comparison | Compares GAZ and competitor models if competitor records exist in the database. | Concrete GAZ models with `manufacturer_brand_lat=GAZ`, and concrete competitor models or brands with competitor `manufacturer_brand_lat` when available in the database. | Factual comparison by price, TTX, body, options, and other structured fields. | Does not do market analysis or search fresh public competitor data. |
