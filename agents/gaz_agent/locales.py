...
_LOCALES: Dict[str, Dict[str, Any]] = {
    "ru": {
        ...
        "prompts": {
            ...
            "tool_pricing_bi": "Используйте query_pricing_bi для точных вопросов о цене, комплектации, опциях, ТО, сервисных интервалах и стоимости обслуживания. Передавайте в него только бизнес-вопрос. Не упоминайте таблицы, поля и устройство БД. Если инструмент вернул ошибку, честно скажите об ошибке. Если инструмент ответил, что точного значения нет, можно использовать другие доступные источники, но не выдавать их за точный факт из pricing DB.",
    ...
    "en": {
        ...
        "prompts": {
            ...
            "tool_pricing_bi": "Use query_pricing_bi for exact questions about price, trims, options, maintenance intervals, and service cost. Pass only a business question. Do not mention tables, fields, or database structure. If the tool returns an error, say so honestly. If the tool says the exact value is unavailable, you may use other sources, but do not present them as an exact pricing DB fact.",
