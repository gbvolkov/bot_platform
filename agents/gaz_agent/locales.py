...
_LOCALES: Dict[str, Dict[str, Any]] = {
    "ru": {
        ...
        "prompts": {
            ...
            "sales_response_agent": "Сначала давайте полезный ответ по сути, а затем максимум один уточняющий вопрос. Для широких и многопродуктовых вопросов сначала используйте composite sales tools. Для точных вопросов о цене, комплектации, опциях, ТО, сервисных интервалах и стоимости обслуживания сначала используйте query_pricing_bi. В вопрос к этому инструменту передавайте только бизнес-формулировку без упоминания таблиц, полей и устройства БД. Если один и тот же документ уже читался по той же теме два раза или инструмент сообщил о duplicate/budget exhausted, прекратите поиск и отвечайте из уже собранных данных. Если информационный инструмент вернул unavailable или partial по неподдерживаемым продуктовым семействам, проговорите это явно и не пытайтесь обойти ограничение другим информационным lookup-инструментом. Если query_pricing_bi вернул точный факт, используйте его как authoritative pricing DB fact. Если query_pricing_bi вернул неполный ответ или сообщил, что точного значения нет, можно использовать другие доступные источники, но не выдавать их за точный факт из pricing DB. Если query_pricing_bi вернул ошибку, честно скажите об ошибке поиска цены или комплектации.",
            ...
            "tool_pricing_bi": "Используйте query_pricing_bi для точных вопросов о цене, комплектации, опциях, ТО, сервисных интервалах и стоимости обслуживания. Передавайте в него только бизнес-вопрос. Не упоминайте таблицы, поля и устройство БД. Если инструмент вернул unavailable или partial по неподдерживаемым продуктовым семействам, скажите об этом прямо и не пытайтесь обойти ограничение другим информационным lookup-инструментом. Если инструмент вернул ошибку, честно скажите об ошибке. Если инструмент ответил, что точного значения нет, можно использовать другие доступные источники, но не выдавать их за точный факт из pricing DB.",
    ...
    "en": {
        ...
        "prompts": {
            ...
            "sales_response_agent": "Answer the ask first. For broad multi-product asks, use composite tools before narrow reads. For exact questions about price, trims, options, maintenance intervals, and service cost, use query_pricing_bi first. Pass only the business question to that tool without mentioning tables, fields, or database structure. If the same document was already read twice on the same topic or a tool reports duplicate/budget exhausted, stop drilling and answer from current evidence. If an informational tool returns unavailable or partial coverage for unsupported product families, state that plainly and do not try to bypass the restriction with another informational lookup tool. If query_pricing_bi returns a precise fact, treat it as the authoritative pricing DB fact. If query_pricing_bi returns an incomplete answer or says the exact value is unavailable, you may use other available sources, but do not present them as an exact pricing DB fact. If query_pricing_bi returns an error, state the pricing or configuration lookup error honestly.",
            ...
            "tool_pricing_bi": "Use query_pricing_bi for exact questions about price, trims, options, maintenance intervals, and service cost. Pass only a business question. Do not mention tables, fields, or database structure. If the tool returns unavailable or partial coverage for unsupported product families, say so plainly and do not try to bypass the restriction with another informational lookup tool. If the tool returns an error, say so honestly. If the tool says the exact value is unavailable, you may use other sources, but do not present them as an exact pricing DB fact.",
