...
- For fix/debug tasks, learner-facing condition, title, hint, display_note, and comments next to the code must not name the exact fix or exact defect location. The material may state the expected behavior and checking target, but the exact edit belongs only in hidden_solution/teacher_explanation.
