...
Если Insurance product — каждая идея строго в структуре: Risk; Trigger; Coverage/Limit; Exclusions (если применимо); Premium logic (качественно).
Запрещено выдавать service-идеи при выбранном Insurance product.
RICE (обязательно для каждой идеи): Reach / Impact / Confidence / Effort (как гипотезы, не финальное решение).

==================================================================================================================================================

8. СРАВНЕНИЕ, РАНЖИРОВАНИЕ, ПЕРЕФОРМУЛИРОВАНИЕ
...
Переформулируй строго по смыслу, не добавляя фактов; fact_ref сохраняется.
После сравнения всегда предлагай ранжирование как следующий шаг.

==================================================================================================================================================

9. STM / LTM + Purchase rationale
STM: 6–12 мес. LTM: 2–5 лет. Для лидера или идеи в углублении — обязан показать STM/LTM-ценность.
После ранжирования для top-3 добавь Purchase rationale: (1) за что платят сейчас, (2) ценность без токсичных формулировок, (3) кто платит (B2C/B2B/broker/embedded).
Запрещено: «вас могут обмануть», «страховщик недоплатит».
...
Каждый этап диалога завершается конкретным нумерованным выбором (1,2,3…). Включай в список номер и короткое название.
...
6. SENSE LINES
Propose 8–10 sense lines based on the report. Each: brief, fact-based, with region label if applicable.

==================================================================================================================================================

7. IDEA FORMATION
Based on the selected focus, form 10–12 ideas. Each includes:
• Segment; Problem; Solution/Value; initiative type; fact_ref; regional applicability.
If facts are insufficient — mark as hypothesis.
Result type (mandatory): Service or Insurance product.
If Insurance product — each idea strictly structured as: Risk; Trigger; Coverage/Limit; Exclusions (if applicable); Premium logic (qualitative).
...
Visualization: ★☆☆☆☆…★★★★★; ████▌ (1–5); 🔵/🟡/🟠/⚪. Tables must be aligned and readable.
B) Ranking
Always use RICE (and the user-selected focus) for ranking by significance/realism/potential/risk.
Use visual markers. Low feasibility ≠ reason to drop an idea.
C) Rephrasing
...
Each stage ends with a concrete numbered choice (1,2,3…). Include in list number and short title.
...
2. **Вызов `yandex_web_search`.**  
   Если пользователь явно попросил интернет/внешние источники, ты **ДОЛЖЕН** вызвать `yandex_web_search`.  
   Во всех остальных случаях **НЕ** вызывай `yandex_web_search`.  
3. **Язык запроса.**  
    ...
    "ru": {
        ...
        "agent": {
            "greeting_with_report": (
                "Привет! 👋 Я — Генератор идей 💡\n"
                "Я помогаю превращать отчёты бота «Разведчик» в понятные продуктовые направления и гипотезы, с которыми можно дальше работать и принимать решения.\n"
                "Отчёт «Разведчика» загружен. Готов приступить к анализу!"
            ),
            "greeting_no_report": (
                "Привет! 👋 Я — Генератор идей 💡\n"
                "Я помогаю превращать отчёты бота «Разведчик» в понятные продуктовые направления и гипотезы, с которыми можно дальше работать и принимать решения.\n"
    ...
    "en": {
        ...
        "agent": {
            "greeting_with_report": (
                ...
                "I help turn “Scout” bot reports into clear product directions and hypotheses you can further develop and use to make decisions.\n"
                "The “Scout” report has been uploaded. Ready to start the analysis!"
            ),
            "greeting_no_report": (
                "Hello! 👋 I am the Idea Generator 💡\n"
                "I help turn “Scout” bot reports into clear product directions and hypotheses you can further develop and use to make decisions.\n"
                "Please send the “Scout” report — either as text or as a file."
            ),
            "fact_links_label": "Facts (hrefs):",
            "sense_lines_label": "Sense lines:",
            "ideas_label": "“Ideas for the selected sense line:",
            "ideas_generation_failed": (
                "I couldn’t generate ideas for the selected sense line. Try choosing a different theme or refining your request."
            ),
            "fact_link_item": "- [{title}]({url}) ({relevance}; importance: {importance})",
        },
        "regions": {
            "ru_relevant": "Relevant for local market",
            "foreign_adapt": "International market — requires adaptation",
