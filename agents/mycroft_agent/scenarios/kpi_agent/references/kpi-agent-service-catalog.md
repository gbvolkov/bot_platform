...
|---|---|---|---|
| Identify Position Context | User asks for "my KPI" or KPI for a role/job/person. | Extract position, department, employee group, and prior context. If no usable clue exists, ask the user for position and department/employee group without calling tools. When fuzzy returns candidates, request the full structure for each candidate before asking the user to choose or confirm. If BI validation leaves exactly one suitable official context, proceed directly to the KPI list without asking for a number or extra confirmation. | `kpi_staff_structure_fuzzy_search` only when a usable structural clue exists; `kpi_bi_int` Staff Structure Validation for every returned `staff_structure_id`, or for exact constraints. |
